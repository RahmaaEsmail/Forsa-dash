...
export default function CustomerInvoiceDetails() {
  ...
  const isApproved = invoice?.status === "approved";
  const isCancelled =
    invoice?.status === "cancelled" || invoice?.status === "canceled";
  const isPaid = invoice?.status === "paid";

  const handlePrint = () => {
    const originalTitle = document.title;
    const documentName = invoice?.invoice_number || id;
  ...
  const formatNumber = (val) =>
    parseFloat(val || 0).toLocaleString("en-US", { maximumFractionDigits: 2 });
  ...
  const columns = [
    {
      ...
      render: (_, record) => {
        ...
        const arabicName =
          record.item?.name && record.item_name !== record.item?.name
            ? record.item.name
            : null;
        return (
          <div className="flex flex-col">
            ...
            {arabicName && (
              <span className="text-xs text-slate-400 font-medium mt-0.5 italic">
                {arabicName}
              </span>
            )}
            {record.sku && (
    ...
    },
    {
      ...
      render: (_, record) => (
        <span className="font-bold text-slate-700 text-sm">
          ...
          {record.unit?.name || record.unit_name || ""}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => (
        <span className="font-extrabold text-primary text-sm">
          {formatAmount(record.line_total || record.subtotal || 0)}{" "}
          {invoice?.currency?.code || "SAR"}
        </span>
      ),
    },
  ];
        ...
        >
            ...
            <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-5 text-xs text-slate-600">
              ...
              <div className="grid grid-cols-2 gap-y-3">
                ...
                <div>
                  ...
                  <span className="text-slate-900 font-bold">
                    {invoice?.currency?.code || "SAR"} (
                    {invoice?.currency?.name || "Saudi Riyal"})
                  </span>
                </div>
              </div>
          ...
          {isCancelled && invoice?.cancellation_reason && (
            <div className="mb-6 p-4 bg-red-50 text-red-800 border border-red-100 rounded-xl text-xs">
              <span className="font-bold">Cancellation Log Reason:</span>{" "}
              ...
              {invoice?.items?.map((item, index) => (
                <tr key={item.id || index} className="align-top">
                  ...
                  <td className="py-4 px-4 text-right font-bold text-slate-900">
                    {formatAmount(item.line_total || item.subtotal || 0)}
                  </td>
                </tr>
              )) || (
                <tr>
            ...
            <div className="w-80 space-y-2 text-xs border-t border-slate-100 pt-4">
              <div className="flex justify-between text-slate-500 px-2">
                ...
                <span className="font-semibold text-slate-900">
                  {formatAmount(invoice?.financial_summary?.subtotal)}{" "}
              ...
              <div className="flex justify-between text-slate-500 px-2">
                <span>Total Discount</span>
                <span className="font-semibold text-red-500">
                  -{formatAmount(invoice?.financial_summary?.discount_amount)}{" "}
                  <span className="text-[10px] text-slate-400 font-normal">
                    SAR
                  </span>
                </span>
              ...
              <div className="flex justify-between text-slate-500 px-2">
                <span>VAT Amount</span>
                <span className="font-semibold text-slate-900">
                  {formatAmount(invoice?.financial_summary?.vat_amount)}{" "}
                  <span className="text-[10px] text-slate-400 font-normal">
                    SAR
                  </span>
                </span>
              ...
              <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-100 text-slate-900 mt-2">
                ...
                <span className="font-black text-base text-primary">
                  {formatAmount(invoice?.financial_summary?.total_amount)}{" "}
