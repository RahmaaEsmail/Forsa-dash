...
import { Edit, FileText, ChevronRight, CheckCircle, XCircle, Printer, Package } from 'lucide-react'
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function GRNDetails() {
  ...
  const totalReceived = grn?.items?.reduce((acc, item) => acc + (Number(item.quantity_received) || 0), 0) || 0;
  const totalAccepted = grn?.items?.reduce((acc, item) => acc + (Number(item.quantity_accepted) || 0), 0) || 0;
  const totalRejected = grn?.items?.reduce((acc, item) => acc + (Number(item.quantity_rejected) || 0), 0) || 0;
  ...
  return (
    <div className="min-h-screen bg-muted/30 p-4 md:p-6">
             ...
             <CardContent className="p-6">
                ...
                <div className="space-y-4 text-sm">
                   ...
                   <div className="flex justify-between items-center py-2 border-b border-slate-50">
                      ...
                      <span className="font-bold text-slate-900">{totalReceived}</span>
                   </div>
                   <div className="flex justify-between items-center py-2 border-b border-slate-50">
      ...
      <div id="printable-grn-area-wrapper" className="bg-white py-6">
        <div className="max-w-[850px] mx-auto bg-white p-12 border border-slate-200 rounded-sm" id="printable-grn-area">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 text-[11px] space-y-1">
              ...
              <p><span className="text-slate-400">Company:</span> <strong className="text-slate-800">{safeText(grn?.rfq?.supplier?.company_name)}</strong></p>
              <p><span className="text-slate-400">VAT No:</span> {safeText(grn?.rfq?.supplier?.vat_number)}</p>
              <p><span className="text-slate-400">Email:</span> {safeText(grn?.rfq?.supplier?.email)}</p>
            </div>

            <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 text-[11px] space-y-1">
              <h4 className="font-bold text-slate-900 text-xs uppercase mb-1">Registry Data</h4>
              <p><span className="text-slate-400">GRN Number:</span> <strong>{safeText(grn?.grn_number)}</strong></p>
              <p><span className="text-slate-400">Received Date:</span> <strong>{formatDate(grn?.received_date)}</strong></p>
              <p><span className="text-slate-400">Linked PO / RFQ:</span> <strong>#{safeText(grn?.rfq?.po_number || grn?.rfq?.rfq_number)}</strong></p>
            </div>
          </div>

          {/* Items Table */}
          <h4 className="text-xs font-bold text-slate-800 tracking-wider uppercase mb-2">Received Items</h4>
              ...
              {grn?.items?.map((item, index) => (
                <tr key={item.id || index} className="align-top">
                  ...
                  <td className="py-3 px-2">
                    <span className="font-bold text-slate-900 block">{item.item?.name || item.item_name}</span>
                    {item.sku && <span className="text-[9px] text-slate-400 font-mono block uppercase">SKU: {item.sku}</span>}
                  </td>
                  <td className="py-3 px-2 text-center text-slate-500">{safeText(item.unit?.name)}</td>
                  <td className="py-3 px-2 text-right font-mono">{item.quantity_expected ?? "—"}</td>
                  <td className="py-3 px-2 text-right font-bold font-mono">{item.quantity_received ?? "—"}</td>
                  <td className="py-3 px-2 text-right font-bold text-emerald-700 font-mono">{item.quantity_accepted ?? "—"}</td>
                  <td className="py-3 px-2 text-right font-bold text-red-600 font-mono">{item.quantity_rejected ?? "—"}</td>
                  <td className="py-3 px-2 text-center">
                    <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded ${
                      item.status === 'accepted' ? 'bg-emerald-50 text-emerald-700' :
                      item.status === 'rejected' ? 'bg-red-50 text-red-700' :
                      'bg-slate-100 text-slate-600'
                    }`}>{safeText(item.status)}</span>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="flex justify-end mb-8">
            <div className="w-72 space-y-1.5 text-[11px] border-t border-slate-100 pt-3">
              <div className="flex justify-between text-slate-500 px-2">
                ...
                <span className="font-semibold text-slate-900 font-mono">{grn?.items?.length ?? 0}</span>
              </div>
              <div className="flex justify-between text-slate-500 px-2">
                <span>Total Received</span>
                <span className="font-semibold text-slate-900 font-mono">{totalReceived}</span>
              </div>
          ...
          {grn?.notes && (
            <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 mb-8 text-[11px]">
              ...
              <p className="text-slate-600 leading-relaxed whitespace-pre-wrap">{grn.notes}</p>
            </div>
          )}
