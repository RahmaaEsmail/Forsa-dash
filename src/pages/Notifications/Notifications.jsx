...
export default function Notifications() {
  ...
  const { data: notifications, isLoading: isListLoading } =
    useListNotifications({ page: currentPage });
    ...
    if (typeof message === "object" && message !== null) {
      return (
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-xs text-muted-foreground italic" dir="rtl">
            {message.ar}
          </span>
        </div>
      );
  ...
  const columns = [
      ...
      render: (_, record) => {
        ...
        return (
          <div className="flex items-center gap-3 py-2">
            ...
            <div
              className={`${isUnread ? "font-bold text-secondary" : "text-muted-foreground font-normal"}`}
            >
    ...
    },
    {
      ...
      align: "left",
      render: (_, record) => (
        <div className="max-w-md">{renderMessage(record.message)}</div>
      ),
    },
    {
      title: "Type",
      ...
      render: (type) => (
        <Badge
          variant="outline"
          className="capitalize bg-muted/30 border-none px-3"
        >
          {type?.replace("_", " ")}
        </Badge>
      ),
