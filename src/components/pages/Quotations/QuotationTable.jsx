...
const statusVariants = {
  ...
  client_approval: "bg-purple-100 text-purple-700 hover:bg-purple-100 border-none",
  sales_manager_approval: "bg-indigo-100 text-indigo-700 hover:bg-indigo-100 border-none",
  proforma_invoice: "bg-orange-100 text-orange-700 hover:bg-orange-100 border-none",
  paid_payment: "bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-none",
...
export default function QuotationTable({ data, isLoading, page, setPage }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (pr) => <Badge variant="outline" className="font-normal">{pr?.pr_number || 'N/A'}</Badge>
    },
    {
      title: "Total Amount",
      dataIndex: "total_amount",
      key: "total_amount",
      render: (val, row) => <span className="font-bold">{val} {row.currency?.code}</span>
    },
    {
      title: "Status",
      dataIndex: "status",
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="icon" onClick={() => navigate(`/quotations/${row.id}/details`)}>
            <Eye className="w-4 h-4 text-slate-500" />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => navigate(`/quotations/${row.id}/edit`)}>
