        ...
        if (res?.data) {
          ...
          const formattedItems =
            rfq.items?.map((item) => ({
              ...
              cost_price:
                Number(item.unit_price) || Number(item.target_price) || 0,
              selling_price:
                (Number(item.unit_price) || Number(item.target_price) || 0) *
                1.2, // Default 20% margin
              tax_rate: Number(item.vat_rate) || defaultTaxSetting,
              available: true,
            })) || [];
        ...
        <div className="p-6 border-b bg-slate-50/50 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs mt-0.5">
              Fill in the basic details for the new quotation
            </p>
          </div>
        </div>
